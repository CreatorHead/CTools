...
{
	if (catch_signal(SIGINT, handle_shutdown) == -1) //This will call handle_shutdown() if Ctrl-C is hit.
	...
	char buf[255]; //Listen for a connection.
	while (1) {
		...
		if(!fork()){
			...
			if (say(connect_d,"Internet Knock-Knock Protocol Server\r\nVersion 1.0\r\nKnock! Knock!\r\n> ") != -1) {	
					...
					if (strncasecmp("Who's there?", buf, 12)) //Checking the user’s answers.
